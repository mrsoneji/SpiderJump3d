...
function M.imgsize (filename, options, baseDir )
    baseDir = baseDir or system.ResourceDirectory
    filename = system.pathForFile( filename, baseDir )
    ...
    if filetype == "string" or filetype == "number" then
        ...
        if not file then
            print("test1 error", filename, err)
    ...
    for pattern, format in pairs(TYPE_MAP) do
        if header:find(pattern) then
            local sizefunc = require( devilsquid.requirepath .. "util.imagesize.imagesize.format." .. format)
