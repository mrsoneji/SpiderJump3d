...
function Arc:new( startAngle, endAngle, radius, strokeWidth, closed, pie )
	...
	arc.lastPercent = 100
	arc.color       = {1,1,1}
	arc.color2      = nil 				-- used for gradients
	...
	function arc:Draw()
		...
		while accumAngle < (self.startAngle + angle) do    
			...
			-- if color 2 is nil then one color
			if self.color2 == nil then
				c:setFillColor( self.color[1],self.color[2],self.color[3] )

			-- if color 2 the calculate a gradient color
			else
				local stepSize = cnt * step
				local diffAngle = math.abs(angle - self.startAngle)

				-- a value betwwen 0 and 1 how much of the drawing is done
				local angleDone = (accumAngle - self.startAngle) / angle

				local rDiff = self.color2[1] - self.color[1]
				local gDiff = self.color2[2] - self.color[2]
				local bDiff = self.color2[3] - self.color[3]
				
				local rStep = self.color[1] + rDiff * angleDone
				local gStep = self.color[2] + gDiff * angleDone
				local bStep = self.color[3] + bDiff * angleDone

				c:setFillColor( rStep,gStep,bStep )
			end
	...
	-- @param redraw = true immediatley redraws the arc after setting the color
	-- @return nothing
	-----------------------------------------------------------------------------
	function arc:SetColor( options )
		log:p(2, "arc:SetColor")
	...
	-----------------------------------------------------------------------------
	-- Sets the second color of the arc for creatig a gradient
	-- @param options table with r,g,b values
	-- @param redraw = true immediatley redraws the arc after setting the color
	-- @return nothing
	-----------------------------------------------------------------------------
	function arc:SetColor2( options )
		log:p(2, "arc:SetColor")
		self.color2 	= {}

		self.color2[1] 	= options.r or 1
		self.color2[2] 	= options.g or 1
		self.color2[3] 	= options.b or 1
	...
	function arc:SetStartAngle( newStartAngle )
		log:p(2,"arc:SetStartAngle(" .. newStartAngle .. ")")
	...
	-- @param percent draws a percentage of the arc, 50 would be the half arc
	...
	function arc:SetPercent( percent, clockwise, bothsides )
		log:p(2,"arc:SetPercent(" .. percent .. ")")

		if percent == 0 then percent = 0.0001 end
