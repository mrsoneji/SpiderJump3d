...
function BitmapCharacter:moveTo(x,y,newHeight)
	...
	if self.image.height == nil then return end
...
function BitmapCharacter:setTintColor(tint)
	...
	-- otherwise tint it.
		 ...
		 -- if self.character is in the list of excludeCharacters table
		 -- then tint it white (no tint)
		if BitmapFont.excludeCharacters ~= nil then
			for i=1,#BitmapFont.excludeCharacters do
				if utf8.char( self.character ) == BitmapFont.excludeCharacters[i] then
					self.image:setFillColor( 1,1,1 )
					return
				end
			end
...
function BitmapString:setText(newText)
	...
	while source:isMore() do 																-- is there more to get ?
		...
		elseif code ~= 13 then 																-- it's a normal character
			...
			if self.insert == nil then return end
...
--//	Sets a table of characters that gets excluded from tinting 
--//	@excludeCharacters 	[table] 		Table of characters
--//	@return nothing

function BitmapString:setExcludeCharacters(excludeCharacters)
	BitmapFont.excludeCharacters = excludeCharacters
end
BitmapString.setExcludeCharactersFromTinting = BitmapString.setExcludeCharacters
